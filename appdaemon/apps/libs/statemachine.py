...
class StateIs(Trigger):
    ...
    def _state_callback(self, unused_entity, unused_attribute, unused_old, new, unused_kwargs):
        if self.active and self._test_predicate(new):
            self.hass.log(
                "statemachine_internal: _state_callback {} - new: {}({})  active: {}  predicate: {}".format(
                    getattr(self, "name"), new, type(new), self.active, self._test_predicate(new)
                )
            )
...
class Machine:
    ...
    """
        ...
        if self.state_entity:
            ...
            if entity_state in {s.name for s in self.states}:
                ...
                self.hass.log("Unrecognized state: {}".format(entity_state), level="WARNING")
    ...
    def _state_callback(self, unused_entity, unused_attribute, unused_old, new, unused_kwargs):
        ...
        if new_state != self.current_state:
            self._perform_transition(
                Transition(to_state=new_state, trigger=None, on_transition=None)
            )
    ...
    def _perform_transition(self, transition):
        ...
        self.hass.log(
            "_perform_transition: {} -> {}".format(from_state, self.current_state), level="WARNING"
        )
        if self.state_entity:
            self.hass.set_state(self.state_entity, state=self.current_state.name)
        # self._start_timer()
    ...
    def _triggered(self, from_state, transition):
        ...
        self.hass.log(
            "_triggered: current {}  from {} --{}---> to {}".format(
                self.current_state, from_state, transition.trigger, transition.to_state
            )
        )
    ...
    """
        ...
        assert isinstance(from_state, self.states), "Invalid state: {}".format(from_state)
        assert isinstance(to_state, self.states), "Invalid state: {}".format(to_state)
        ...
        trigger.initialize(self.hass, partial(self._triggered, from_state, transition))
        if self.current_state == from_state:
    ...
    def add_transitions(self, from_states, triggers, to_state, on_transition=None):
    ...
    """
        ...
        if (
            from_states != ANY
            and not isinstance(from_states, list)
            and not isinstance(triggers, list)
        ):
    ...
    def log_graph_link(self):
        ...
        link = "https://dreampuf.github.io/GraphvizOnline/#{}".format(quote(self.get_dot()))
